...
public class EMChatManagerRepository extends BaseEMRepository {
    ...
    public void fetchConversationsFromServer(IMV2Callback<IMActionResult> callback){
        EMClient.getInstance().chatManager().asyncFetchConversationsFromServer(new EMValueCallBack<Map<String, EMConversation>>() {
            ...
            public void onSuccess(Map<String, EMConversation> value) {
                ...
                callback.onEvent(new IMActionResult.Success(infoList));
            }

            @Override
            public void onError(int error, String errorMsg) {
                callback.onEvent(new IMActionResult.Failure(error, errorMsg));
            }
        });
    }
    ...
    public LiveData<Resource<List<EaseConversationInfo>>> fetchConversationsFromServer(int pageNum, int pageSize) {
        return new NetworkOnlyResource<List<EaseConversationInfo>>() {
