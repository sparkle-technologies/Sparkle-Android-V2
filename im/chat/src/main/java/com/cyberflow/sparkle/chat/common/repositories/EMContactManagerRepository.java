...
public class EMContactManagerRepository extends BaseEMRepository {
    ...
    public LiveData<Resource<Boolean>> acceptInvitation(String username) {
        return new NetworkOnlyResource<Boolean>() {

            @Override
            protected void createCall(@NonNull ResultCallBack<LiveData<Boolean>> callBack) {
                getContactManager().asyncAcceptInvitation(username, new EMCallBack() {
                    @Override
                    public void onSuccess() {
                        callBack.onSuccess(new MutableLiveData<>(true));
                    }
    ...
    public LiveData<Resource<Boolean>> declineInvitation(String username) {
        return new NetworkOnlyResource<Boolean>() {
...
//                }
    ...
    public LiveData<Resource<EaseUser>> fetchUserInfoByAttribute(String userId) {
        return new NetworkOnlyResource<EaseUser>() {
            ...
            protected void createCall(@NonNull ResultCallBack<LiveData<EaseUser>> callBack) {
                ...
                EMClient.getInstance().userInfoManager().fetchUserInfoByAttribute(userIds, userInfoTypes, new EMValueCallBack<Map<String, EMUserInfo>>() {
                    ...
                    public void onSuccess(Map<String, EMUserInfo> value) {
                        if (callBack != null) {
                            callBack.onSuccess(createLiveData(transformEMUserInfo(value.get(userId))));
                        }
                    }
