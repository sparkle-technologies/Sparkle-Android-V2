...
public class EaseDefaultEmojiconDatas {
    ...
    private static final EaseEmojicon[] DATA = createData1();

    private static EaseEmojicon[] createData() {
        EaseEmojicon[] datas = new EaseEmojicon[icons.length];
        for (int i = 0; i < icons.length; i++) {
            datas[i] = new EaseEmojicon(icons[i], systemEmojis[i], Type.NORMAL);
        }
        return datas;
    }

    private static EaseEmojicon[] createData1() {
        // 0x1F600 to  0x1F64F
        int start = Integer.parseInt("1F600", 16);
        int end = Integer.parseInt("1F64F", 16);
        int totalSize = end - start + 1;
        Log.e("TAG", "createData1: start="+ start  + "\t end=" + end  );
        EaseEmojicon[] datas = new EaseEmojicon[end - start + 1];
        for (int i = 0 ; i < totalSize; i++) {
            datas[i] = new EaseEmojicon(new String(Character.toChars(i + start)));
        }
        return datas;
    }
