...
public class EaseChatLayout extends RelativeLayout implements
        IChatLayout, IHandleMessageView, IPopupWindow, ChatInputMenuListener,
        EMMessageListener, EaseChatMessageListLayout.OnMessageTouchListener,
        ...
        EaseChatInputMenu.OnConversationInputPanelStateChangeListener {
...
//        }
        ...
        if (chatType != EaseConstant.CHATTYPE_SINGLE) return;
...
//        inputMenu.showExtendMenu(false);
    ...
    public void inputAtUsername(String username, boolean autoAddAtSymbol) {
        if (EMClient.getInstance().getCurrentUser().equals(username) || !messageListLayout.isGroupChat()) {
            return;
        }
        ...
        if (autoAddAtSymbol) insertText(editText, AT_PREFIX + username + AT_SUFFIX);
        else insertText(editText, username + AT_SUFFIX);
    }
            ...
            public boolean onMenuItemClick(MenuItemBean item) {
                ...
                if (showDefaultMenu) {
                    ...
                    if (itemId == R.id.action_chat_copy) {
                        clipboard.setPrimaryClip(ClipData.newPlainText(null, ((EMTextMessageBody) message.getBody()).getMessage()));
    ...
    private boolean showTranslation(EMMessage message) {
        if (!EMClient.getInstance().translationManager().isInitialized()) return false;
        ...
        if (result.showTranslation()) return false;
